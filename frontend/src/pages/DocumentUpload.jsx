...
import { useDocumentAnalyzeMutation, useComplexDocumentAnalyzeMutation, useDocumentStatusQuery } from '../hooks/useClaimQueries';
import { useDocumentStore } from '../stores/useDocumentStore';
import { buildGraphFromExtraction, buildComplexGraphFromExtraction } from '../services/assetApi';
...
export default function DocumentUpload() {
  ...
  const [uploadMode, setUploadMode] = useState('single'); // 'single' or 'complex'

  const analyzeMutation = useDocumentAnalyzeMutation();
  const complexAnalyzeMutation = useComplexDocumentAnalyzeMutation();
  ...
  const handleAnalyze = () => {
    ...
    const mutation = uploadMode === 'complex' ? complexAnalyzeMutation : analyzeMutation;
    mutation.mutate(file, {
      onSuccess: (data) => {
        ...
        if (data.success && data.data) {
          if (uploadMode === 'complex') {
            setSuccess(true);
            buildComplexGraphFromExtraction('demo', data.data)
              .catch(err => console.error('Failed to update complex graph:', err));
            return;
          }
      ...
      <main className="max-w-xl mx-auto w-full px-6 py-8 flex-1 relative z-10">
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg w-fit mx-auto mb-6">
              <button
                ...
                className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${uploadMode === 'single' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Financial Claim
              </button>
              <button
                onClick={() => setUploadMode('complex')}
                className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${uploadMode === 'complex' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Complex Family Document
              </button>
            </div>
          </div>
        )}
        ...
        {!editableFields && !success && (
          <>
            ...
            {/* Complex Mode Success State */}
            {success && uploadMode === 'complex' && (
              <div className="mt-8 text-center bg-emerald-50 border border-emerald-100 p-6 rounded-xl">
                ...
                <h3 className="text-lg font-extrabold text-emerald-800 mb-2">Extraction Complete</h3>
                <p className="text-emerald-600 text-sm mb-6">The family network and assets have been seamlessly integrated into your Asset Map.</p>
                <Link to="/asset-discovery" className="inline-flex items-center justify-center rounded-lg bg-slate-900 hover:bg-slate-800 text-white font-bold px-6 h-11 transition-all shadow-sm">
                  View Asset Map
                </Link>
              </div>
            )}

            {/* Interactive Preview for Single Mode */}
            {editableFields && uploadMode === 'single' && (
              <div className="mt-8">
                <AIAnalysisCard 
                  data={editableFields} 
                  file={file} 
            ...
            {/* Trust Flow Progress */}
            {uploading && (
              <div className="mb-6 space-y-3 bg-slate-50 border border-slate-100 p-4 rounded-xl">
                ...
                <div className="flex justify-between text-[10px] uppercase tracking-wider font-bold text-slate-400 px-1">
                  <span className={progress >= 10 ? 'text-emerald-600 transition-colors duration-500' : ''}>Upload</span>
                  <span className={progress >= 30 ? 'text-emerald-600 transition-colors duration-500' : ''}>Secure Processing</span>
                  <span className={progress >= 60 ? 'text-emerald-600 transition-colors duration-500' : ''}>AI Analysis</span>
                  <span className={progress >= 90 ? 'text-emerald-600 transition-colors duration-500' : ''}>Protected Data</span>
                </div>
        ...
        {/* Data Ownership & Security Indicators */}
        <div className="mt-8">
          ...
          <div className="text-center">
            <p className="text-xs text-slate-400 font-medium tracking-wide">
              <span className="inline-block mr-1">🔒</span> Your family information belongs to you. VARASAT helps you understand and organize it.
            </p>
          </div>
        </div>
